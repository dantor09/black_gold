...
function get_mysqli_connection() {
    ...
    if (!isset($connection)) {
        $connection = mysqli_connect(
            ...
            'paypool', // the MariaDB username
            'loopyap3420F22', // the MariaDB username's password
            'paypool' // the MariaDB database name
...
function get_pdo_connection() {
    ...
    if (!isset($conn)) {
        try {
            ...
            $conn = new PDO(
                "mysql:host=localhost;dbname=paypool",  // change dbname
                "paypool",                          // change username
                "loopyap3420F22",                      // change password
                $options);
        }
